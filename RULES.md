...
### 2026-02-14 22:58
- דרישה/שינוי: fix: normalize REQUIRED SCRIPTS to real filenames (push.sh etc.) + dedupe
- בוצע בפועל: תוקן כללים.md: סעיף REQUIRED SCRIPTS כולל רק שמות קבצים אמיתיים ללא תוספות/סוגריים (כולל scripts/push.sh). RULES.md מסונכרן. build_rules + rules_check עברו.
- המשך מתוכנן: הבא: שלח צילום מסך של Download/page-1.pdf כדי לכוון מ״מ (ריווחים/גובה שורה/שבירות) עד הדפסה מושלמת.
